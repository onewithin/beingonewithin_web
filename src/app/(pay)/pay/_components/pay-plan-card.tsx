...
import { SubscriptionPlan, createWebCheckoutWithToken } from "@/lib/actions/subscription";
...
export default function PayPlanCard({ plan, isMonthly, token, disableSubscribe = false }: PayPlanCardProps) {
    ...
    const proceedToCheckout = async () => {
        ...
        try {
            ...
            const result = await createWebCheckoutWithToken(token, plan.id, {
                successUrl: `${origin}/pay?token=${encodeURIComponent(token)}&payment=success&session_id={CHECKOUT_SESSION_ID}`,
                cancelUrl: `${origin}/pay?token=${encodeURIComponent(token)}&payment=cancel`,
            });
    ...
    const getIntervalText = () => {
        if (plan.intervalCount === 1) return `/ ${plan.interval}`;
            ...
            <Card className={`${colors.bg} border-2 ${colors.border} rounded-2xl overflow-hidden`}>
                <CardContent className="p-3 sm:p-6">
                    <div className="text-center mb-3 sm:mb-6">
                        <h3 className="text-[#121212] text-[13px] sm:text-lg mb-2 sm:mb-4 font-poppins-600 leading-tight">
                            {plan.name}
                        </h3>
                        <div className="mb-1 sm:mb-2 font-sniglet-400">
                            <span className={`text-2xl sm:text-4xl ${colors.price}`}>
                                {formatPrice(plan.price)}
                            </span>
                            <span className="text-[#484848] text-xs sm:text-lg ml-1">
                                {getIntervalText()}
                            </span>
                        </div>
                        {!isMonthly && (
                            <p className="text-[#484848] text-[11px] sm:text-sm font-poppins-400">⭐ Best Value ⭐</p>
                        )}
                        {isMonthly && (
                            <p className="text-[#484848] text-[11px] sm:text-sm font-poppins-400">Most flexible</p>
                        )}
                        {plan.trialDays && plan.trialDays > 0 && (
                            <p className="text-[#121212] text-[11px] sm:text-sm font-poppins-600 mt-1 sm:mt-2">
                    ...
                    <Button
                        ...
                        disabled={loading}
                        className={`w-full font-poppins-600 px-3 sm:px-6 py-3 sm:py-5 text-[12px] sm:text-base ${colors.button} text-white font-semibold rounded-xl mb-2 sm:mb-4`}
                    >
                        {getButtonText()}
                    </Button>
                    <p className="text-[#484848] font-poppins-400 text-[10px] sm:text-xs text-center leading-relaxed">

...
function TopicsListLoadingOverlay() {
    ...
    useEffect(() => {
        const onStart = (event: Event) => {
            ...
            if (typeof customEvent.detail === 'string' && customEvent.detail.length > 0) {
                setPendingTopicId(customEvent.detail)
            }
        }

        window.addEventListener('topics:topic-switch-start', onStart as EventListener)
        return () => window.removeEventListener('topics:topic-switch-start', onStart as EventListener)
    }, [])
    ...
    useEffect(() => {
        ...
        if (currentTopicId === pendingTopicId) {
            setPendingTopicId(null)
        }
    }, [searchParams, pendingTopicId])
...
export default TopicsListLoadingOverlay
